...
class ResnetBlock(nn.Module):
    def __init__(self, dim, dim_out, *, time_emb_dim=None, age_emb_dim=None, groups=6):
        super().__init__()
        ...
        # dedicated per-block age FiLM (decoupled from the timestep embedding).
        # zero-initialised so a freshly-loaded pretrained model is unchanged until trained.
        self.age_film = nn.Linear(age_emb_dim, dim_out * 2) if exists(age_emb_dim) else None
        if self.age_film is not None:
            nn.init.zeros_(self.age_film.weight); nn.init.zeros_(self.age_film.bias)
    ...
    def forward(self, x, time_emb=None, age_emb=None):
        ...
        if exists(self.mlp):
            ...
            if (age_emb is not None) and (self.age_film is not None):
                ae = rearrange(self.age_film(age_emb), 'b c -> b c 1 1 1')
                a_scale, a_shift = ae.chunk(2, dim=1)
                scale = scale + a_scale; shift = shift + a_shift
...
class BiFlowNet(nn.Module):
    def __init__(
        ...
        if self.age_condition:
            # continuous age (in [0,1]) -> sinusoidal -> MLP -> time_dim, added to t like the class emb
            self.age_mlp = nn.Sequential(
                SinusoidalPosEmb(dim),
                nn.Linear(dim, time_dim), nn.SiLU(),
                nn.Linear(time_dim, time_dim))
            self.null_age = nn.Parameter(torch.zeros(time_dim))   # CFG "unconditional" token
        ...
        block_klass_cond = partial(block_klass, time_emb_dim=time_dim,
                                   age_emb_dim=(dim * 4 if self.age_condition else None))
    ...
    def forward(
        ...
        age_film_emb = None                                    # dedicated per-block age FiLM input
        if self.age_condition and age is not None:
            drop = (age < 0).view(-1, 1)                       # age<0 => CFG-dropped (unconditional)
            a = torch.clamp(age, min=0.0).view(-1) * 1000.0    # [0,1] -> sinusoidal range
            age_emb = self.age_mlp(a)
            age_emb = torch.where(drop, self.null_age.to(age_emb.dtype).expand_as(age_emb), age_emb)
            t = t + age_emb
            t_DiT = t_DiT + age_emb.unsqueeze(1).repeat(1, p1*p2*p3, 1).view(-1, c)
            age_film_emb = age_emb
        ...
        for idx, (block1, spatial_attn1, block2, spatial_attn2,downsample) in enumerate(self.downs):
            if idx <self.feature_fusion :
                x = x + h_Unet.pop(0)
            x = block1(x, t, age_film_emb)
            x = spatial_attn1(x)
            h.append(x)
            x = block2(x, t, age_film_emb)
            x = spatial_attn2(x)
            h.append(x)
            x = downsample(x)

        x = self.mid_block1(x, t, age_film_emb)
        x = self.mid_spatial_attn(x)
        x = self.mid_block2(x, t, age_film_emb)
...
class GaussianDiffusion(nn.Module):
    ...
    def p_losses(self, denoise_fn,x_start, t, y=None, res=None,noise=None, hint = None, age=None,**kwargs):
        ...
        if is_list_str(y):
            ...
            y = y.to(device)
        if hint == None:
            x_recon = denoise_fn(x_noisy, t, y=y,res=res, age=age)
        else:
            x_recon = denoise_fn(x_noisy, t, y=y, res=res,hint = hint, age=age)
        # time_rel_pos_bias = self.time_rel_pos_bias(x.shape[2], device=x.device)
